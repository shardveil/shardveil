...
const abi = cardRegistryAbi;
type ContractAbi = typeof abi;

function useContractAddress() {
  const chainId = useChainId();
  return chainId === ARBITRUM_SEPOLIA_CHAIN_ID
    ? addresses[ARBITRUM_SEPOLIA_CHAIN_ID].cardRegistry
    : undefined;
}

export function useCardRegistryRead<
...
export function useCardRegistryWrite() {
  const address = useContractAddress();
...
export function useCardRegistryEvents<
...
export { cardRegistryAbi as CardRegistryAbi };
export type { ContractAbi as CardRegistryContractAbi };
