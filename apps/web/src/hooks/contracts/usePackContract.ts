...
const abi = packContractAbi;
type ContractAbi = typeof abi;

function useContractAddress() {
  const chainId = useChainId();
  return chainId === ARBITRUM_SEPOLIA_CHAIN_ID
    ? addresses[ARBITRUM_SEPOLIA_CHAIN_ID].packContract
    : undefined;
}

export function usePackContractRead<
...
export function usePackContractWrite() {
  const address = useContractAddress();
...
export function usePackContractEvents<
...
export { packContractAbi as PackContractAbi };
export type { ContractAbi as PackContractContractAbi };
