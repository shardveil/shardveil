...
const abi = craftingEngineAbi;
type ContractAbi = typeof abi;

function useContractAddress() {
  const chainId = useChainId();
  return chainId === ARBITRUM_SEPOLIA_CHAIN_ID
    ? addresses[ARBITRUM_SEPOLIA_CHAIN_ID].craftingEngine
    : undefined;
}

export function useCraftingEngineRead<
...
export function useCraftingEngineWrite() {
  const address = useContractAddress();
...
export function useCraftingEngineEvents<
...
export { craftingEngineAbi as CraftingEngineAbi };
export type { ContractAbi as CraftingEngineContractAbi };
