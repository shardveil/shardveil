...
const abi = battleEngineAbi;
type ContractAbi = typeof abi;

function useContractAddress() {
  const chainId = useChainId();
  return chainId === ARBITRUM_SEPOLIA_CHAIN_ID
    ? addresses[ARBITRUM_SEPOLIA_CHAIN_ID].battleEngine
    : undefined;
}

export function useBattleEngineRead<
...
export function useBattleEngineWrite() {
  const address = useContractAddress();
...
export function useBattleEngineEvents<
...
export { battleEngineAbi as BattleEngineAbi };
export type { ContractAbi as BattleEngineContractAbi };
